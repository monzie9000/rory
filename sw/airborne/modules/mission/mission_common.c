...
bool_t mission_insert(enum MissionInsertMode insert, struct _mission_element *element)
{
    ...
    if (!mission_element_convert(element))
    {
        return FALSE;
    }
    ...
    case Append:
        ...
        if (tmp == mission.current_idx)
        {
            return FALSE;    // no room to insert element
        }
        mission.elements[mission.insert_idx] = *element; // add element
        mission.insert_idx = tmp; // move insert index
        break;
    case Prepend:
        if (mission.current_idx == 0)
        {
            tmp = MISSION_ELEMENT_NB - 1;
        }
        else
        {
            tmp = mission.current_idx - 1;
        }
        if (tmp == mission.insert_idx)
        {
            return FALSE;    // no room to inser element
        }
...
bool_t __attribute__((weak)) mission_element_convert(struct _mission_element *el __attribute__((unused)))
{
    return TRUE;
}
...
void mission_status_report(void)
{
    ...
    if (j == 0)
    {
        task_list[j++] = 0;    // Dummy value if task list is empty
    }
...
int mission_parse_GOTO_WP(void)
{
    ...
    {
        return FALSE;    // not for this aircraft
    }
...
{
    if (DL_MISSION_GOTO_WP_LLA_ac_id(dl_buffer) != AC_ID)
    {
        return FALSE;    // not for this aircraft
    }
    ...
    if (!mission_point_of_lla(&me.element.mission_wp.wp.wp_f, &lla))
    {
        return FALSE;
    }
...
{
    if (DL_MISSION_CIRCLE_LLA_ac_id(dl_buffer) != AC_ID)
    {
        return FALSE;    // not for this aircraft
    }
    ...
    me.type = MissionCircle;
    // if there is no valid local coordinate, do not insert mission element
    if (!mission_point_of_lla(&me.element.mission_circle.center.center_f, &lla))
    {
        return FALSE;
    }
...
{
    if (DL_MISSION_SEGMENT_LLA_ac_id(dl_buffer) != AC_ID)
    {
        return FALSE;    // not for this aircraft
    }
    ...
    me.type = MissionSegment;
    // if there is no valid local coordinate, do not insert mission element
    if (!mission_point_of_lla(&me.element.mission_segment.from.from_f, &from_lla))
    {
        return FALSE;
    }
    if (!mission_point_of_lla(&me.element.mission_segment.to.to_f, &to_lla))
    {
        return FALSE;
    }
...
int mission_parse_PATH(void)
{
    ...
    if (me.element.mission_path.nb > MISSION_PATH_NB)
    {
        me.element.mission_path.nb = MISSION_PATH_NB;
    }
...
{
    if (DL_MISSION_PATH_LLA_ac_id(dl_buffer) != AC_ID)
    {
        return FALSE;    // not for this aircraft
    }
    ...
    {
        // if there is no valid local coordinate, do not insert mission element
        if (!mission_point_of_lla(&me.element.mission_path.path.path_f[i], &lla[i]))
        {
            return FALSE;
        }
    }
...
{
    if (DL_GOTO_MISSION_ac_id(dl_buffer) != AC_ID)
    {
        return FALSE;    // not for this aircraft
    }
...
{
    if (DL_NEXT_MISSION_ac_id(dl_buffer) != AC_ID)
    {
        return FALSE;    // not for this aircraft
    }
    ...
    if (mission.current_idx == mission.insert_idx)
    {
        return FALSE;    // already at the last position
    }
...
{
    if (DL_END_MISSION_ac_id(dl_buffer) != AC_ID)
    {
        return FALSE;    // not for this aircraft
    }
