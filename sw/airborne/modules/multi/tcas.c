                          ...
                          for (i = 2; i < NB_ACS; i++)
{
    if (the_acs[i].ac_id == 0)
        ...
        if (dt > TCAS_DT_MAX)
        {
            continue;    // lost com but keep current status
        }
              ...
              if (tcas_status == TCAS_RA)
{
    ...
    tcas_resolve = tcas_test_direction(tcas_ac_RA);
        ...
        else     // second resolution or message received
        {
            ...
            {
                if (tcas_resolve == RA_CLIMB && the_acs[the_acs_id[tcas_ac_RA]].climb > 1.0)
                {
                    tcas_resolve = RA_DESCEND;    // revert resolve
                }
                else if (tcas_resolve == RA_DESCEND && the_acs[the_acs_id[tcas_ac_RA]].climb < -1.0)
                {
                    tcas_resolve = RA_CLIMB;    // revert resolve
                }
            }
        }
        // Downlink alert
