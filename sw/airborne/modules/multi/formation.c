...
{
    if (_id != AC_ID && the_acs_id[_id] == 0)
    {
        return FALSE;    // no info for this AC
    }
...
int formation_flight(void)
{
    ...
    if (formation[the_acs_id[AC_ID]].status != ACTIVE)
    {
        return FALSE;    // AC not ready
    }
