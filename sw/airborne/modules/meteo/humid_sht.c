...
uint8_t s_write_byte(uint8_t value)
{
    ...
    for (i = 0x80; i > 0; i /= 2)         //shift bit for masking
    {
        if (i & value)
        {
            DATA_SET;    //masking value with i , write to SENSI-BUS
        }
        else
        {
            DATA_CLR;
...
uint8_t s_read_byte(uint8_t ack)
{
    ...
    if (ack)
    {
        DATA_CLR;    //in case of "ack==1" pull down DATA-Line
    }
    SCK_SET;                              //clk #9 for ack
    SCK_SET;
    SCK_SET;
...
uint8_t s_measure(uint16_t *p_value, uint8_t *p_checksum, uint8_t mode)
{
    ...
    for (i = 0; i < 6665535; i++) if (DATA_IN == 0)
        {
