...
#error RADIO_PAN or RADIO_ROLL not defined.
            ...
            else if (cam_theta < RadOfDeg(CAM_TILT_MIN))
            {
                cam_theta = RadOfDeg(CAM_TILT_MIN);
            }
            ...
            else if (cam_phi < RadOfDeg(CAM_PAN_MIN))
            {
                cam_phi = RadOfDeg(CAM_PAN_MIN);
            }
...
//cam_theta = RadOfDeg(70);     //  LOOK 45 DEGREES DOWN, 0 IS STRAIGHT DOWN 90 IS STRAIGHT IN FRONT
            ...
            if (heading_radians > RadOfDeg(180))
            {
                heading_radians -= RadOfDeg(360);
            }
            if (heading_radians < RadOfDeg(-180))
            {
                heading_radians += RadOfDeg(360);
            ...
            if ((float)(*fbw_state).channels[RADIO_CAM_LOCK] > MAX_PPRZ / 2)) && pprz_mode == PPRZ_MODE_AUTO2)
            {
                cam_lock = TRUE;
            }
            if ((float)(*fbw_state).channels[RADIO_CAM_LOCK] < MIN_PPRZ / 2 && pprz_mode == PPRZ_MODE_AUTO2)
        {
            cam_lock = FALSE;
        }
#endif
