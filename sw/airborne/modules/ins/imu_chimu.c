...
static CHIMU_attitude_data GetEulersFromQuat(CHIMU_attitude_data attitude)
{
    ...
    if (ps.euler.phi < 0)
    {
        ps.euler.phi = ps.euler.phi + 2 * M_PI;
    }
                                   ...
                                   CHIMU_PARSER_DATA *pstData)
{
    ...
    case CHIMU_Msg_0_Ping:
        CHIMU_index = 0;
        pstData->gCHIMU_SW_Exclaim = pPayloadData[CHIMU_index];
        CHIMU_index++;
        pstData->gCHIMU_SW_Major = pPayloadData[CHIMU_index];
        CHIMU_index++;
        pstData->gCHIMU_SW_Minor = pPayloadData[CHIMU_index];
        CHIMU_index++;
        pstData->gCHIMU_SW_SerialNumber = (pPayloadData[CHIMU_index] << 8) & (0x0000FF00);
        CHIMU_index++;
        pstData->gCHIMU_SW_SerialNumber += pPayloadData[CHIMU_index];
        CHIMU_index++;
    ...
    case CHIMU_Msg_3_IMU_Attitude:
        ...
        pstData->gCalStatus = pPayloadData[CHIMU_index];
        CHIMU_index ++;
        pstData->gCHIMU_BIT = pPayloadData[CHIMU_index];
        CHIMU_index ++;
        pstData->gConfigInfo = pPayloadData[CHIMU_index];
        CHIMU_index ++;
